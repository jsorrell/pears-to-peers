...
RoomManager.prototype.startNewGame = function()
{
    ...
    var that = this;
    this.gameManager =
    new GameManager(this.playerInfo,
                    this.playerIds,
                    leader,
                    function(winner){
            if (winner){
            ...
            var scoreMessage = new Message.Message("RoomMessage","scores",{scores: that.scores});
...
function GameManager(playerInfo, playerIds, leader, callback){
//DATA MEMBERS:
...
{
    var message = new Message.Message("GameMessage","gameOver");
...
GameManager.prototype.broadcast = function(message,except)
{
  ...
  //     this.playerInfo[player].getSocket().send(JSON.stringify(notification));
  // }
}
...
GameManager.prototype.deletePlayer = function(playerId) {
    this.numPlayers --;
    if (this.leaderId === playerId || this.numPlayers < consts.minPlayers) {
        //stop this round and restart
        this.endGame(false);
        return;
    }
...
function attachServerManagerEvents(serverManager) {
  ...
  serverManager.on('createRoom', function(data, socket){
    ...
    if (roomId in this.roomInfo) {
        var errorMsg = new Message.Message("ServerMessage","Error");
    ...
    var response = new Message.Message("ServerMessage","roomList",{roomList: this.rooms});
    this.broadcast(response);
